    ...
    if([UIImagePickerController isSourceTypeAvailable:type]) {
        ...
        if(type == UIImagePickerControllerSourceTypeCamera) {
            ...
            if(imgType && imgType.outline.length > 0) {
                [[SDWebImageManager sharedManager] downloadImageWithURL:[NSURL URLWithString:[[DefMediaHost stringByAppendingString:@"/"] stringByAppendingString:imgType.outline]] options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
    ...
    if(indexPath.row == self.curProduct.photos.count) {
        ...
        if(self.curProduct.photos != nil && self.curProduct.photos.count >= (indexPath.row + 1)) {
            ...
            if(photo != nil && [photo isKindOfClass:[Photo class]]) {
                ...
                [newCell.photoView sd_setImageWithURL:[NSURL URLWithString:[NSURL URLWithString:[DefMediaHost stringByAppendingString:imgType.preview]]] placeholderImage:[UIImage imageNamed:@"stub"]];
            }
        } else {
            [newCell.photoView sd_setImageWithURL:[NSURL URLWithString:[NSURL URLWithString:[DefMediaHost stringByAppendingString:imgType.preview]]] placeholderImage:[UIImage imageNamed:@"stub"]];
        }
